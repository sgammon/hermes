...
the standard directories:
    ...
    *  ``handlers``:
       Contains :py:class:`webapp2.RequestHandler`-descendent classes
       that are bound to one-or-many URLs and are responsible for
       handling a single HTTP request/response lifecycle. All you have
       to do to make one is extend `handlers.WebHandler` and define
       your HTTP methods as, well... methods::
    ...
    *  ``messages``:
       This directory holds structured :py:class:`protorpc.message.Message`
       classes that express request or response datastructures for the
       ``apptools`` service layer. Since apptools models can be used
       transparently as messages, only explicitly-defined or custom-built
       message classes need to live here. These are directly ``ProtoRPC``-
